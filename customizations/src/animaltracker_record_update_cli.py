"""
CLI entrypoint for Animal Tracker record updates (step 3).

Mirrors the multi-db/alias behavior of the other Animal Tracker CLIs.
"""
...
import animaltracker_record_update_sync


def _run_for_alias(alias: str) -> None:
    banner = alias or "(default)"
    print(f"== Animal Tracker record update: {banner} ==")
    ...
    if MULTIPLE_DATABASES_TYPE == "map":
        ...
        if len(MULTIPLE_DATABASES_MAP) == 0:
            al.warn(
                ...
                "animaltracker_record_update_cli",
                None,
            )
            dbo = db.get_dbo()
            dbo.timeout = 0
